...
public class TicTacToe extends JFrame implements ActionListener{
    ...
    ImageIcon masterIcon=new ImageIcon(this.getClass().getResource("/TicTacToeAssets/O.png"));
    CheckBoardCondition check=new CheckBoardCondition();
        ...
        for(int i=0;i<buttons.length;i++)
        {
            if(e.getSource()==buttons[i])
            {
                ...
                {                                       //If that square has not been played, perform the play and
                    ...
                    boardState[i] = turnPlayer;         //hold turn at current player and print error to status bar.
                    ...
                    }else if(check.checkDrawCondition(boardState))
                    {
                        setStatus("The game is a draw!!!");

                        int confirm=JOptionPane.showOptionDialog(null, String.format("The game is a " +
                                        "draw!!!\nWould you like to play again?"),String.format("Game Draw!"),
                                JOptionPane.YES_NO_OPTION,JOptionPane.INFORMATION_MESSAGE,null,null,
                                null);
                        if(confirm!=1) //showOptionDialog returns 0 for yes and 1 for no
                        {
